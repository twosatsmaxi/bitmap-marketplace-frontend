...
export function drawBitfeedVacuum(
  ...
  for (let i = 0; i < squares.length; i++) {
    ...
    const dist = (canvasSize / gridSize) * (1.2 + Math.cos(i * 789.1) * 0.5);
    const startX = sq.x + Math.cos(angle) * dist;
    const startY = sq.y + Math.sin(angle) * dist;
    ...
    // Vacuum-like ease: slow start, then snap
    const eased = easeOutBack(currentProgress);
    
    const curX = startX + (sq.x - startX) * eased;
    const curY = startY + (sq.y - startY) * eased;
